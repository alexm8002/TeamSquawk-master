...
- (void)drawWithFrame:(NSRect)cellFrame inView:(NSView *)controlView
{
	...
	// Draw gradient when in recording mode
		...
		// Draw snapback image
            ...
            [snapBackArrow drawAtPoint:[self _snapbackRectForFrame: cellFrame].origin fromRect:NSZeroRect operation:NSCompositingOperationXOR fraction:1.0f];
	...
	// Draw border and remove badge if needed
		...
		// If key combination is set and valid, draw remove image
			if (![self _isEmpty] && [self isEnabled])
			{
                ...
                [removeImage drawAtPoint:[self _removeButtonRectForFrame: cellFrame].origin fromRect:cellFrame operation:NSCompositingOperationSourceOver fraction:1.0f];
			}
		}
...
//		NSLog(@"stroked along path of %@", NSStringFromRect(correctedSnapBackRect));
		...
		// Draw snapback image
			NSImage *snapBackArrow = SRResIndImage(@"SRSnapback");
			[snapBackArrow drawAtPoint:correctedSnapBackOrigin fromRect:cellFrame operation:NSCompositingOperationSourceOver fraction:1.0f*alphaRecording];
		}
	...
	// Draw border and remove badge if needed
		...
		if (![self _isEmpty] && [self isEnabled])
		{
			NSString *removeImageName = [NSString stringWithFormat: @"SRRemoveShortcut%@", (mouseInsideTrackingArea ? (mouseDown ? @"Pressed" : @"Rollover") : (mouseDown ? @"Rollover" : @""))];
			NSImage *removeImage = SRResIndImage(removeImageName);
			[removeImage drawAtPoint:[viewportMovement transformPoint:([self _removeButtonRectForFrame: cellFrame].origin)] fromRect:cellFrame operation:NSCompositingOperationSourceOver fraction:alphaView];
